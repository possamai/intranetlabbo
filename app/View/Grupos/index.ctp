...
<div class="row-fluid">
                    ...
                    <table class="table table-bordered">
                        <thead>
                          <tr class="head-table">
                                ...
                                <th><?PHP echo $this->Paginator->sort('parent_id', '[Id] Grupo Superior<span class="order"></span>', array('escape' => false)); ?></th>
                                <th><?PHP echo $this->Paginator->sort('usuario_id', 'Gerente<span class="order"></span>', array('escape' => false)); ?></th>
                                <th style="width: 100px;">Total Usuários</th>
                                <th><?php echo __('Ações'); ?></th>
                            </tr>                    
                            <tr class="filter">
                            	<th><?PHP echo $this->Form->input('titulo', array('label'=>false)); ?></th>
                            	<th><?PHP echo $this->Form->input('parent_id', array('label'=>false,'empty' => '')); ?></th>
                            	<th><?PHP echo $this->Form->input('usuario_id', array('label'=>false,'type' => 'text')); ?></th>
                                <th>&nbsp;</th>
                                <th>&nbsp;</th>
                            </tr>
                        </thead>
	...
	<tr>
		...
		<td><?php echo $this->Html->link(  (($grupo['ParentGrupo']['id']<>'')?$grupo['ParentGrupo']['id']. ' - ':'') . $grupo['ParentGrupo']['titulo'], array('controller' => 'grupos', 'action' => 'cadastro', $grupo['ParentGrupo']['id'])); ?></td>
		<td><?php echo $this->Html->link($grupo['Gerente']['nome'], array('controller' => 'usuarios', 'action' => 'cadastro', $grupo['Gerente']['id'])); ?></td>
    	<td><?php echo ( (count($grupo['Usuario'])) ? $this->Html->link( count( $grupo['Usuario'] ), array('controller' => 'usuarios', 'action' => 'index', 'grupo'=>$grupo['Grupo']['id'], 'status'=>2)) : '' ); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link( '<i class="icon16 icomoon-icon-pencil mr0"></i>', array('action' => 'cadastro', $grupo['Grupo']['id']), array('title'=>'Editar', 'class'=>'btn','escape' => false)); ?>
			<?php echo $this->Html->link( '<i class="icon16 icomoon-icon-remove mr0"></i>', array('action' => 'excluir', $grupo['Grupo']['id']), array('title'=>'Excluir', 'class'=>'btn confirmLink', 'escape' => false)); ?>
		</td>
	</tr>
<?php endforeach; ?>
